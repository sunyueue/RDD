        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast():
                ...
                # EDD Stage
                    ...
                    VGD_lamda = (iteration - args.max_iterations) / (warmup_iter - args.max_iterations)
                    # args.max_iterations
                    if self.args.aux:
                        task_loss_EDD = self.OHEMcriterion.forward_with_teacher_student(s_outputs[0], t_outputs[0], targets) \
                                    + 0.4 * self.OHEMcriterion.forward_with_teacher_student(s_outputs[1], t_outputs[1],
                                                                                            targets)
                        task_loss_VGD = self.CEcriterion(s_outputs[0], targets) + 0.4 * self.CEcriterion(s_outputs[1],
                                                                                                     targets)
                        variance = torch.sum(kl_distance(log_sm(t_outputs[0]), sm(t_outputs[1])), dim=1)
                        exp_variance = torch.exp(-variance)
                        exp_variance_scale = torch.unsqueeze(exp_variance, 1)
                        ...
                        task_loss_VGD = torch.mean(task_loss_VGD * exp_variance_scale)
                        task_loss = task_loss_EDD * (1 - VGD_lamda) + task_loss_VGD * VGD_lamda

                    else:
                        task_loss_EDD = self.OHEMcriterion.forward_with_teacher_student(s_outputs[0], t_outputs[0], targets)

                        task_loss_VGD = self.CEcriterion(s_outputs[0], targets)
                        variance = torch.sum(kl_distance(log_sm(t_outputs[0]), sm(t_outputs[1])), dim=1)
                        exp_variance = torch.exp(-variance)
                        exp_variance_scale = torch.unsqueeze(exp_variance, 1)
                        exp_variance_scale = F.interpolate(exp_variance_scale, (512, 1024), mode='bilinear',
