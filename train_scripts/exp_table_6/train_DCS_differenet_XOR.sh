		...
		do    
			...
			do
				...
				python -m torch.distributed.launch --nproc_per_node=${nproc_per_node} --master_port 29501\
					...
					--save-dir "work_dirs/table6/deeplabv3_res18_onlykd_TwoStage_diff_bs${batch_size}_iteration_${max_iterations}_warmupiter_${warmup_iteration}_EDD_method_${EDD_method}_repeat_${label}" \
					--log-dir "work_dirs/table6/deeplabv3_res18_onlykd_TwoStage_diff_bs${batch_size}_iteration_${max_iterations}_warmupiter_${warmup_iteration}_EDD_method_${EDD_method}_repeat_${label}" \
