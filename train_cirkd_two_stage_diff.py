...
class Trainer(object):
    def __init__(self, args):
        ...
        self.OHEMcriterion = OhemCrossEntropy2d(ignore_index=args.ignore_label).to(self.device)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast():
                ...
                # Stage1: 先学 anti-uncertainty 的内容
                if iteration < warmup_iter:
                    ...
                    if self.args.aux:
                        task_loss = self.OHEMcriterion.forward_with_teacher_student(s_outputs[0], t_outputs[0], targets) \
                                    + 0.4 * self.OHEMcriterion.forward_with_teacher_student(s_outputs[1], t_outputs[1],
                                                                                            targets)
                    else:
                        task_loss = self.OHEMcriterion.forward_with_teacher_student(s_outputs[0], t_outputs[0], targets)
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                ...
                writer.add_scalar('Train/Loss_minibatch_pixel_contrast', minibatch_pixel_contrast_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_memory_pixel_contrast', memory_pixel_contrast_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_memory_region_contrast', memory_region_contrast_loss_reduced.item(), iteration)
